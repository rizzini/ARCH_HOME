...
if [[ "$(/bin/df -B MB  /dev/sda2 --output=avail | /usr/bin/tail -1 | /usr/bin/tr -d 'MB')" -le 700 || "$1" == 'force' && "$2" != 'allhdd' ]];then
    out_of_space=1;
    if [ "$2" == 'allssd' ];then
        /sbin/btrfs sub del /mnt/archlinux/btrbk_snapshots/HOME/* | tee -a /tmp/clean_space_emergency.log;
        /sbin/btrfs sub del /mnt/archlinux/btrbk_snapshots/ROOT/* | tee -a /tmp/clean_space_emergency.log;
        /sbin/btrfs sub del /mnt/archlinux/btrbk_snapshots/BOOT_ESP/* | tee -a /tmp/clean_space_emergency.log;
        ...
        done < <(/usr/bin/find /mnt/archlinux/btrbk_snapshots/HOME/ -prune -type d) | tee -a /tmp/clean_space_emergency.log
        while IFS= read -r d;do
            if [[ -d "$d" && "$d" != *"$(/sbin/btrfs subvol list / | /bin/grep ROOT | /usr/bin/awk '{print $9}' | /usr/bin/tail -1)"* ]];then
                /sbin/btrfs sub del "$d";
            fi
        done < <(/usr/bin/find /mnt/archlinux/btrbk_snapshots/ROOT/ -prune -type d) | tee -a /tmp/clean_space_emergency.log
        while IFS= read -r d;do
            if [[ -d "$d" && "$d" != *"$(/sbin/btrfs subvol list / | /bin/grep BOOT_ESP | /usr/bin/awk '{print $9}' | /usr/bin/tail -1)"* ]];then
                /sbin/btrfs sub del "$d";
            fi
        done < <(/usr/bin/find /mnt/archlinux/btrbk_snapshots/BOOT_ESP/ -prune -type d) | tee -a /tmp/clean_space_emergency.log
        while IFS= read -r d;do
...
if [[ "$(/bin/df -B MB  "$(/bin/mount | /bin/grep  '/mnt/backup' | /usr/bin/awk '{print $1}')" --output=avail | /usr/bin/tail -1 | /usr/bin/tr -d 'MB')" -le 700 || "$1" == 'force' && "$2" != 'allssd' ]];then
    ...
    if [ "$2" == 'allhdd' ];then
        ...
        /sbin/btrfs sub del /mnt/backup/ROOT/* | tee -a /tmp/clean_space_emergency.log;
        /sbin/btrfs sub del /mnt/backup/BOOT_ESP/* | tee -a /tmp/clean_space_emergency.log;
    else
        while IFS= read -r d;do
